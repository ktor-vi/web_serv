...
if ($argc > 1) {
    $word = $argv[1];  // Récupérer le premier argument (fichier demandé)
} else {
    $word = getenv("WORD") ?: "MISSING";  // Sinon, utiliser la variable d'environnement
}

$word = trim($word);  // Nettoyer les espaces superflus
...
$result = str_replace("{}", $word, $phrases[array_rand($phrases)]);
